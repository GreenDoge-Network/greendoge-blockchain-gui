...
const PY_FOLDER = '../greendoge/daemon';
...
const getGreenDogeVersion = () => {
  let version = null;
  const exePath = getExecutablePath('greendoge');
  // first see if we can get a greendoge exe in a standard location relative to where we are
  try {
    ...
    // that didn't work, let's try as if we're in the venv or greendoge is on the path
    try {
      version = child_process
